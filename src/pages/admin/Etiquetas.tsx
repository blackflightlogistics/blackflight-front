      ...
      <main className="md:ml-64 h-full overflow-y-auto bg-[#fcf8f5] p-6 space-y-6 pt-16 md:pt-6 print:p-4 print:overflow-visible">
        ...
        <div ref={pdfRef} className="space-y-6">
          <section className="border p-4 rounded bg-white shadow">
            <h2 className="text-lg font-semibold mb-2">{t.etiqueta_remetente}</h2>
            <p>{remetente.name}</p>
            <p className="text-sm text-gray-600">
              {remetente.addresses[0].city} - {remetente.addresses[0].state} - {remetente.addresses[0].country}
            </p>

            <h2 className="text-lg font-semibold mt-4 mb-2">{t.etiqueta_destinatario}</h2>
            <p>{destinatario.name}</p>
            <p className="text-sm text-gray-600">
              {destinatario.addresses[0].city} - {destinatario.addresses[0].state} - {destinatario.addresses[0].country}
            </p>
          ...
          <section className="border p-6 rounded bg-white shadow flex flex-col items-center justify-center">
            <h2 className="text-lg font-semibold mb-4">{t.etiqueta_codigo_encomenda}</h2>
          ...
          {(pacotesParaImprimir ?? []).length > 0 && (
            <section className="border p-4 rounded bg-white shadow print:break-before-page">
              <h2 className="text-lg font-semibold mb-4">{t.etiqueta_codigos_pacotes}</h2>
